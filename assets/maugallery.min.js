...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImageIndex=null;let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".gallery img.gallery-item").each(function(){imagesCollection.push($(this))})}else{$(".gallery img[data-gallery-tag='"+activeTag+"']").each(function(){imagesCollection.push($(this))})}
imagesCollection.forEach(function(img,index){if(img.attr("src")===$(".lightboxImage").attr("src")){activeImageIndex=index}});let prevImageIndex=activeImageIndex-1;if(prevImageIndex<0){prevImageIndex=imagesCollection.length-1}
let prevImage=imagesCollection[prevImageIndex];$(".lightboxImage").attr("src",prevImage.attr("src"))},nextImage(){let activeImageIndex=null;let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".gallery img.gallery-item").each(function(){imagesCollection.push($(this))})}else{$(".gallery img[data-gallery-tag='"+activeTag+"']").each(function(){imagesCollection.push($(this))})}
imagesCollection.forEach(function(img,index){if(img.attr("src")===$(".lightboxImage").attr("src")){activeImageIndex=index}});let nextImageIndex=activeImageIndex+1;if(nextImageIndex>=imagesCollection.length){nextImageIndex=0}
let nextImage=imagesCollection[nextImageIndex];$(".lightboxImage").attr("src",nextImage.attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
